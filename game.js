...
const ALFAJORES_DATA = {
  ...
  blanco: {
    id: 'blanco', nombre: 'Guaymallen Blanco', costo: 500, clickValue: 2, aps: 0.5, emoji: '⚪',
    mordiscos: 4,
    images: {
...
const EDIFICIOS_DATA = {
  kiosquero:  { id: 'kiosquero',  nombre: 'Kiosquero',           desc: 'El pibe de la esquina vende tus alfajores', precioBase: 15,     aps: 0.1,   emoji: '🏪', owned: 0 },
  panaderia:  { id: 'panaderia',  nombre: 'Panaderia',           desc: 'Pan y alfajores, combo ideal',              precioBase: 100,    aps: 0.5,   emoji: '🥖', owned: 0 },
  maquina:    { id: 'maquina',    nombre: 'Maquina DDL',         desc: 'Dulce de leche non-stop',                   precioBase: 500,    aps: 2.0,   emoji: '⚙️', owned: 0 },
  linea:      { id: 'linea',      nombre: 'Linea de chocolate',  desc: 'El banado perfecto',                        precioBase: 2000,   aps: 8.0,   emoji: '🍫', owned: 0 },
  camion:     { id: 'camion',     nombre: 'Camioncito',          desc: 'Reparte por todo el pais',                  precioBase: 8000,   aps: 25.0,  emoji: '🚚', owned: 0 },
  fabrica:    { id: 'fabrica',    nombre: 'Fabrica',             desc: 'La fabrica de Mataderos',                   precioBase: 30000,  aps: 80.0,  emoji: '🏭', owned: 0 },
  exportadora:{ id: 'exportadora',nombre: 'Exportadora',         desc: 'Alfajores al mundo',                        precioBase: 150000, aps: 300.0, emoji: '✈️', owned: 0 },
  fabrica2:   { id: 'fabrica2',   nombre: 'Planta Nacional',     desc: 'Segunda planta para maxima produccion',     precioBase: 500000, aps: 800.0, emoji: '🏗️', owned: 0 }
};
  ...
  Object.values(EDIFICIOS_DATA).forEach(edif => {
    [
      ...
      mejoras.push({
        ...
        comprado: false, requiere: { edificio: edif.id, cantidad: tier.cantReq }
      });
    });
  });
  return mejoras;
...
function showGoldenAlfajor() {
  ...
  if (!container) { goldenActive = false; scheduleGoldenAlfajor(); return; }
  const golden = document.createElement('div');
  golden.id = 'goldenAlfajor';
  golden.className = 'golden-alfajor';
  golden.innerHTML = '🏆<br><span>ALFAJOR DORADO</span><br><small>+DDL Bonus</small>';
  const maxX = window.innerWidth  - 120;
  const maxY = window.innerHeight - 120;
  golden.style.left = Math.max(60, Math.random() * maxX) + 'px';
  golden.style.top  = Math.max(60, Math.random() * maxY) + 'px';
  golden.addEventListener('pointerdown', claimGoldenAlfajor);
  container.appendChild(golden);
  setTimeout(() => {
    if (golden.parentNode) { golden.remove(); goldenActive = false; scheduleGoldenAlfajor(); }
  }, 8000);
}
...
// ==================== TIENDA ====================
function renderShop() { renderAlfajoresShop(); renderEdificiosShop(); renderMejorasShop(); }
...
function renderAlfajoresShop() {
  ...
  Object.values(ALFAJORES_DATA).forEach(alf => {
    ...
    item.innerHTML = '<div class="item-icon">' + alf.emoji + '</div><div class="item-info"><div class="item-name">' + alf.nombre + '</div><div class="item-description">' + (alf.catchphrase || 'Alfajor clasico argentino') + '</div><div class="item-stats"><span>+' + alf.clickValue + ' por clic</span><span>+' + alf.aps + ' APS</span></div></div><div class="item-right"><div class="item-price ' + (canBuy || unlocked ? '' : 'price-locked') + '">' + (unlocked ? (isEquipped ? 'EQUIPADO' : 'EQUIPAR') : (alf.costo === 0 ? 'GRATIS' : formatNumber(alf.costo))) + '</div></div>';
...
function renderEdificiosShop() {
  ...
  const sorted = Object.values(EDIFICIOS_DATA).sort((a, b) => {
    ...
    if (cA && !cB) return -1; if (!cA && cB) return 1; return pA - pB;
  });
  sorted.forEach(edif => {
    ...
    item.innerHTML = '<div class="item-icon">' + edif.emoji + '</div><div class="item-info"><div class="item-name">' + edif.nombre + (owned > 0 ? ' <span class="owned-badge">x' + owned + '</span>' : '') + '</div><div class="item-description">' + edif.desc + '</div><div class="item-stats"><span>' + edifAps + ' APS/u</span>' + (owned > 0 ? '<span>' + formatNumber(totalContrib, 1) + ' APS total</span>' : '') + '</div></div><div class="item-right"><div class="item-price ' + (canBuy ? '' : 'price-locked') + '">' + formatNumber(precio) + '</div><div class="buy-label">x' + buyAmount + '</div></div>';
...
function renderMejorasShop() {
  ...
  if (disponibles.length === 0 && compradas.length === 0) { container.innerHTML = '<div class="no-mejoras">Compra productores para desbloquear mejoras</div>'; return; }
  disponibles.forEach(m => {
    const canBuy = gameState.alfajores >= m.costo;
    const item = document.createElement('div');
    item.className = 'mejora-item' + (canBuy ? ' can-buy' : '');
    item.innerHTML = '<div class="mejora-icon">⬆️</div><div class="mejora-info"><div class="mejora-name">' + m.nombre + '</div><div class="mejora-desc">' + m.desc + '</div></div><div class="mejora-price ' + (canBuy ? '' : 'price-locked') + '">' + formatNumber(m.costo) + '</div>';
    if (canBuy) item.addEventListener('click', () => buyMejora(m.id));
    container.appendChild(item);
  });
  if (compradas.length > 0) {
    const sep = document.createElement('div'); sep.className = 'mejoras-sep'; sep.textContent = 'Mejoras compradas'; container.appendChild(sep);
    compradas.forEach(m => {
      const item = document.createElement('div'); item.className = 'mejora-item comprada';
      item.innerHTML = '<div class="mejora-icon">OK</div><div class="mejora-info"><div class="mejora-name">' + m.nombre + '</div><div class="mejora-desc">' + m.desc + '</div></div>';
      container.appendChild(item);
    });
  }
}
...
function getEdificioMultiplier(id) {
  let mult = 1;
  MEJORAS_DATA.forEach(m => { if (gameState.mejoras[m.id] && m.tipo === 'edificio' && m.target === id && m.multiplicador > mult) mult = m.multiplicador; });
  return mult;
}
...
function preloadAlfajorImages(id) {
  const alf = ALFAJORES_DATA[id];
  if (!alf) return;
  Object.values(alf.images).forEach(src => { if (!imageCache[src]) { const img = new Image(); img.src = src; imageCache[src] = img; } });
}
...
function buyEdificio(id) {
  ...
  if (gameState.alfajores >= precio) {
    ...
    calculateAPS(); renderShop(); updateUI(); saveGame(); shopNeedsUpdate = false;
  }
}

function calculateAPS() {
  let totalAps = 0;
  if (gameState.alfajoresUnlocked[gameState.currentAlfajor]) totalAps += ALFAJORES_DATA[gameState.currentAlfajor].aps;
  Object.entries(gameState.edificios).forEach(([id, count]) => { if (count > 0 && EDIFICIOS_DATA[id]) totalAps += EDIFICIOS_DATA[id].aps * count * getEdificioMultiplier(id); });
...
function checkMejoras() {
  ...
  if (badge) { badge.textContent = nuevas.length; badge.style.display = nuevas.length > 0 ? 'inline-block' : 'none'; }
}
...
function switchTab(tabName) {
  ...
  if (tabName === 'mejoras') { const badge = document.getElementById('mejorasBadge'); if (badge) badge.style.display = 'none'; renderMejorasShop(); }
}

function startGameLoop() {
  setInterval(() => {
    const now = Date.now(); const deltaTime = (now - gameState.lastUpdate) / 1000; gameState.lastUpdate = now;
    const earned = gameState.aps * deltaTime;
    if (earned > 0) { gameState.alfajores += earned; gameState.totalAlfajores += earned; updateUI(); shopNeedsUpdate = true; }
  }, 100);
  setInterval(() => { if (shopNeedsUpdate) { shopNeedsUpdate = false; renderEdificiosShop(); renderAlfajoresShop(); checkMejoras(); } }, 500);
...
function saveGame() {
  localStorage.setItem('guaymallenClicker', JSON.stringify({
    ...
    clickMultiplier: gameState.clickMultiplier, currentAlfajor: gameState.currentAlfajor,
    alfajoresUnlocked: gameState.alfajoresUnlocked, edificios: gameState.edificios,
    mejoras: gameState.mejoras, goldenCount: gameState.goldenCount, lastSave: Date.now()
  }));
}
...
function loadGame() {
    ...
    try {
      ...
      if (data.lastSave) {
        ...
        if (offlineEarnings > 1) {
          gameState.alfajores += offlineEarnings; gameState.totalAlfajores += offlineEarnings;
  ...
  if (Object.keys(gameState.mejoras).length > 0) {
    ...
    MEJORAS_DATA.forEach(m => { if (gameState.mejoras[m.id] && m.tipo === 'click') gameState.clickMultiplier *= m.multiplicador; });
  }
  calculateAPS();
  updateAlfajorImage();
}
...
if ('serviceWorker' in navigator) {
  window.addEventListener('load', () => {
    navigator.serviceWorker.register('sw.js').then(() => console.log('SW OK')).catch(err => console.log('SW error:', err));
  });
       }// ==================== DATOS DEL JUEGO ====================
const ALFAJORES_DATA = {
    negro: {
...
const EDIFICIOS_DATA = {
    kiosquero:  { id: 'kiosquero',  nombre: 'Kiosquero',           desc: 'El pibe de la esquina vende tus alfajores', precioBase: 15,     aps: 0.1,   emoji: '🏪', owned: 0 },
    panaderia:  { id: 'panaderia',  nombre: 'Panadería',           desc: 'Pan y alfajores, combo ideal',              precioBase: 100,    aps: 0.5,   emoji: '🥖', owned: 0 },
    maquina:    { id: 'maquina',    nombre: 'Máquina DDL',         desc: 'Dulce de leche non-stop',                   precioBase: 500,    aps: 2.0,   emoji: '⚙️', owned: 0 },
    linea:      { id: 'linea',      nombre: 'Línea de chocolate',  desc: 'El bañado perfecto',                        precioBase: 2000,   aps: 8.0,   emoji: '🍫', owned: 0 },
    camion:     { id: 'camion',     nombre: 'Camioncito',          desc: 'Reparte por todo el país',                  precioBase: 8000,   aps: 25.0,  emoji: '🚚', owned: 0 },
    fabrica:    { id: 'fabrica',    nombre: 'Fábrica',             desc: 'La fábrica de Mataderos',                   precioBase: 30000,  aps: 80.0,  emoji: '🏭', owned: 0 },
    exportadora:{ id: 'exportadora',nombre: 'Exportadora',         desc: 'Alfajores al mundo',                        precioBase: 150000, aps: 300.0, emoji: '✈️', owned: 0 },
    fabrica2:   { id: 'fabrica2',   nombre: 'Planta Nacional',     desc: 'Segunda planta para máxima producción',     precioBase: 500000, aps: 800.0, emoji: '🏗️', owned: 0 }
};

// Mejoras escalables: generadas dinámicamente + base
const MEJORAS_BASE = [
  { id: 'click_x2',      nombre: 'Clic hambriento',    desc: 'Cada clic vale el doble',       costo: 200,   tipo: 'click', multiplicador: 2, comprado: false, requiere: { totalAlfajores: 50 } },
  { id: 'click_x4',      nombre: 'Furia alfajorera',   desc: 'Cada clic vale 4 veces más',    costo: 5000,  tipo: 'click', multiplicador: 4, comprado: false, requiere: { totalAlfajores: 2000 } },
  { id: 'click_x8',      nombre: 'Clic bestia',        desc: 'Cada clic vale 8 veces más',    costo: 50000, tipo: 'click', multiplicador: 8, comprado: false, requiere: { totalAlfajores: 20000 } },
  { id: 'click_x16',     nombre: 'Modo dios alfajor',  desc: 'Cada clic vale 16 veces más',   costo: 500000,tipo: 'click', multiplicador: 16, comprado: false, requiere: { totalAlfajores: 200000 } }
  ];

// Genera mejoras de edificios automáticamente (escalable)
function generarMejorasEdificios() {
    const mejoras = [];
    const edificios = Object.values(EDIFICIOS_DATA);
          ...
          [
            ...
            { suffix: '_x8', mult: 8, cantReq: 25, costMult: 250 }
                        ...
                        mejoras.push({
                                  id: edif.id + tier.suffix,
                                  nombre: edif.nombre + ' +' + (tier.mult * 100 - 100) + '%',
                                  desc: edif.nombre + ' produce ' + tier.mult + 'x más',
                                  costo: Math.floor(edif.precioBase * tier.costMult),
                                  tipo: 'edificio',
                                  target: edif.id,
...
// ==================== ALFAJOR DORADO ====================
function scheduleGoldenAlfajor() {
    const delay = (Math.random() * 60 + 30) * 1000; // 30-90 segundos
  goldenTimeout = setTimeout(showGoldenAlfajor, delay);
}
...
function showGoldenAlfajor() {
  ...
  golden.addEventListener('click', claimGoldenAlfajor);
    golden.addEventListener('touchend', (e) => { e.preventDefault(); claimGoldenAlfajor(); });
    container.appendChild(golden);

  // Desaparece en 8 segundos si no lo clickean
  setTimeout(() => {
        if (golden.parentNode) { golden.remove(); goldenActive = false; scheduleGoldenAlfajor(); }
  }, 8000);
}
...
function renderAlfajoresShop() {
    ...
    Object.values(ALFAJORES_DATA).forEach(alf => {
          ...
          if (!unlocked && !canBuy) item.classList.add('locked');
          if (isEquipped) item.classList.add('equipped');
          item.innerHTML = `<div class="item-icon">${alf.emoji}</div>
                <div class="item-info">
                        <div class="item-name">${alf.nombre}</div>
                                <div class="item-description">${alf.catchphrase || 'Alfajor clásico argentino'}</div>
                                        <div class="item-stats"><span>👆 +${alf.clickValue} por clic</span><span>⏱️ +${alf.aps} APS</span></div>
                                              </div>
                                                    <div class="item-right">
                                                            <div class="item-price ${canBuy || unlocked ? '' : 'price-locked'}">${unlocked ? (isEquipped ? '✅ EQUIPADO' : '🔄 EQUIPAR') : (alf.costo === 0 ? 'GRATIS' : '🍫 ' + formatNumber(alf.costo))}</div>
                                                                  </div>`;
          if (!unlocked && canBuy) item.addEventListener('click', () => buyAlfajor(alf.id));
          else if (unlocked && !isEquipped) item.addEventListener('click', () => equipAlfajor(alf.id));
          container.appendChild(item);
...
function renderEdificiosShop() {
    ...
    sorted.forEach(edif => {
          ...
          const edifAps = edif.aps * getEdificioMultiplier(edif.id);
          const totalContrib = edifAps * owned;
          item.innerHTML = `<div class="item-icon">${edif.emoji}</div>
                <div class="item-info">
                        <div class="item-name">${edif.nombre} ${owned > 0 ? '<span class="owned-badge">x' + owned + '</span>' : ''}</div>
                                <div class="item-description">${edif.desc}</div>
                                        <div class="item-stats"><span>⏱️ ${edifAps} APS/u</span>${owned > 0 ? '<span>📊 ' + formatNumber(totalContrib, 1) + ' APS total</span>' : ''}</div>
                                              </div>
                                                    <div class="item-right">
                                                            <div class="item-price ${canBuy ? '' : 'price-locked'}">🍫 ${formatNumber(precio)}</div>
                                                                    <div class="buy-label">Comprar x${buyAmount}</div>
                                                                          </div>`;
          if (canBuy) item.addEventListener('click', () => buyEdificio(edif.id));
          container.appendChild(item);
    });
...
function renderMejorasShop() {
    ...
    if (disponibles.length === 0 && compradas.length === 0) {
          container.innerHTML = '<div class="no-mejoras">🔒 Comprá productores para desbloquear mejoras</div>';
          return;
    }
    disponibles.forEach(m => {
          const canBuy = gameState.alfajores >= m.costo;
          const item = document.createElement('div');
          item.className = 'mejora-item' + (canBuy ? ' can-buy' : '');
          item.innerHTML = `<div class="mejora-icon">⬆️</div>
    ...
    if (compradas.length > 0) {
          ...
          compradas.forEach(m => {
                  ...
                  item.innerHTML = `<div class="mejora-icon">✅</div><div class="mejora-info"><div class="mejora-name">${m.nombre}</div><div class="mejora-desc">${m.desc}</div></div>`;
                  container.appendChild(item);
          });
    }
}
...
function getEdificioMultiplier(id) {
    ...
    MEJORAS_DATA.forEach(m => {
          if (gameState.mejoras[m.id] && m.tipo === 'edificio' && m.target === id) {
                  // solo aplicar el multiplicador más alto de cada tier
            if (m.multiplicador > mult) mult = m.multiplicador;
          }
    });
    return mult;
}
...
function loadGame() {
          ...
          try {
                  ...
                  if (data.lastSave) {
                            ...
                            if (offlineEarnings > 1) {
                                        ...
                                        setTimeout(() => showNotificacion('🕐 Producción offline', 'Ganaste ' + formatNumber(offlineEarnings) + ' alfajores mientras estabas ausente!'), 500);
                            }
                  }
    ...
    if (Object.keys(gameState.mejoras).length > 0) {
          gameState.clickMultiplier = 1;
          MEJORAS_DATA.forEach(m => {
                  if (gameState.mejoras[m.id] && m.tipo === 'click') gameState.clickMultiplier *= m.multiplicador;
          });
    }
    calculateAPS();
    updateAlfajorImage();
